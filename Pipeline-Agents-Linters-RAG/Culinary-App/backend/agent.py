...
def parse_recipe_from_response(text: str):
    ...
    name_match = re.search(r'NAZWA:\s*(.+)', text, re.IGNORECASE)
    if name_match:
        name = name_match.group(1).strip().split("\n")[0].strip()

    # Blok do następnej sekcji (wiele linii pod SKŁADNIKI)
    ingredients_match = re.search(
        r'SKŁADNIKI:\s*(.+?)(?=\s*KROKI\s*:|$)',
        text,
    ...
    if ingredients_match:
        ...
        ingredients = [p.strip() for p in raw_parts if p.strip()]

    steps_match = re.search(r'KROKI:\s*(.+)', text, re.DOTALL | re.IGNORECASE)
    if steps_match:
...
def run_agent(message: str, thread_id: str, user_id: int) -> dict:
    result = graph.invoke(
    ...
    if final_response:
        ...
        if ingredients or steps:
            recipe = save_recipe(user_id, name, ingredients, steps)
            recipe_id = recipe.id
            if image_filename:
